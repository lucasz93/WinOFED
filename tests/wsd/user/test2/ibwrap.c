...
int create_mr(struct qp_pack *qp, struct mr_pack *mr, ib_access_t acl) {
...
{
	VAPI_rr_desc_t rwr;
	VAPI_sg_lst_entry_t dataseg[1];
	ib_api_status_t status;

	/* Post receive buffer on qp1 */
	memset(&rwr, 0, sizeof(rwr));
	rwr.id          = (VAPI_wr_id_t)(MT_virt_addr_t)qp;
	rwr.opcode      = VAPI_RECEIVE;
	rwr.comp_type = VAPI_SIGNALED;

	rwr.sg_lst_len  = 1;
	rwr.sg_lst_p    = dataseg;

	dataseg[0].addr = (unsigned long)mr->buf;
	dataseg[0].len = mr->size;
	dataseg[0].lkey = mr->rep_mrw.l_key;

	/* Post the work request */
    vret = VAPI_post_rr(qp->hca_handle, qp->qp_handle, &rwr);

	cl_atomic_inc(&qp->wq_posted);

	if (vret != VAPI_OK) {
		VAPI_ERR_LOG("VAPI_post_rr failed ", vret);
		atomic_dec(&qp->wq_posted);
		goto fail;
	}
