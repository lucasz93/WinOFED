...
if "%T %" == "hang" (
    echo %T%: hang test, iterations 100000
    set STIME=!DATE! !TIME!
    %DT% -T T -s %S% -D %D% -i 1000 -t 2 -w 2 client SR 256 server RW 4096 server SR 256 client SR 256 server RW 4096 server SR 256 client SR 4096 server SR 256 client RW 4096 1 server RW 2048 4 client SR 1024 4 server SR 4096 2 client SR 1024 3 -f server SR 2048 1 -f server RR 8192 1 client RR 8192 1
    set ETIME=!DATE! !TIME!
    goto xit
)
...
if "%T%" == "ping" (
    rem Connectivity test - client sends one buffer of 32 bytes, one time.
    rem add '-d' before 'client' for debug output.
    echo Simple Connectivity test
    set CMD=%DT% -T T -s %S% -D %D% -i 1 -t 1 -w 1 -d client SR 32 server SR 32
    goto xcmd
)
...
if "%T%" == "transmete" (
    ...
    set CMD=%DT% -T T -P -t 4 -w 4 -i 8192 -s %S% -D %D% client RW  4096 1 server RW  2048 4  server RR  1024 2 client RR 2048 2 client SR 1024 3 -f server SR 256 3 -f
    goto xcmd
)
...
if "%T%" == "perf" (
    ...
    set CMD=%DT% -T P %DBG% -s %S% -D %D% -i 100000 -p 32 RW 4096 2
    goto xcmd
)

if "%T%" == "rdma-read" (
    echo %T% 4 32K segs
    set CMD=%DT% -T P -s %S% -D %D% -i 10000 -p 32 RR 32768 4
    goto xcmd
)

if "%T%" == "rdma-write" (
    echo %T% 4 32K segs
    set CMD=%DT% -T P -s %S% -D %D% -i 10000 -p 32 RW 32768 4
    goto xcmd
)

if "%T%" == "bw" (
    echo bandwidth 4096 iterations of 2 65K mesgs
    set CMD=%DT% -T P -s %S% -D %D% -i 10000 -p 32 -m p RW 65536 2 
    goto xcmd
)

if "%T%" == "latb" (
    echo latency test - block for completion events
    set CMD=%DT% -T P -s %S% -D %D% -i 10000 -p 1 -m b RW 4 1
    goto xcmd
)

if "%T%" == "latp" (
    echo latency test - poll completion events
    set CMD=%DT% -T P -s %S% -D %D% -i 10000 -p 1 -m p RW 4 1
    goto xcmd
)
...
if "%T%" == "regression" (
    ...
    set RT=trans perf threads threadsm transm transt transme transmet transmete rdma-write rdma-read bw EP EPA
	set STIME=!DATE! !TIME!
