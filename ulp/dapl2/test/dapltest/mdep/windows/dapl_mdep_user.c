...
bool
DT_Mdep_GetCpuStat ( DT_CpuStat *cpu_stat )
{
    BOOL      status;
    LONG64    cycleTime;

    status = QueryProcessCycleTime( GetCurrentProcess(), &cycleTime );
    if( status == 0 )
    {
	fprintf(stderr,"%s() QueryProcessCycleTime() failed %#x\n",
			__FUNCTION__, GetLastError());
    }
    cpu_stat->system = 0;
    cpu_stat->user = cycleTime;	// combined user+system for this process..sigh.
    cpu_stat->idle = 0;
...
{
    return GetTickCount(); // milliseconds since boot, up to 49.7 days.
}

double
DT_Mdep_GetCpuMhz ( void )
{
    static double   dapl_cpuSpeed = 0.0;

    if( dapl_cpuSpeed == 0.0 )
    {
    	LONG    retVal;
    	HKEY    hKey;
        ...
        RegCloseKey (hKey);
	dapl_cpuSpeed = (double) cpuSpeed;
    }

    return dapl_cpuSpeed;
}


#if 0	// XXX Not used as this PCFreq 2922.3920 differed from above MHz 2992.0 ?

double
XDT_Mdep_GetCpuMhz ( void )
{
    ...
    if( PCFreq == 0.0 )
    {
         ...
         {
             fprintf(stderr,"%s() QueryPerformanceFrequency() failed?\n",
			__FUNCTION__);
             return 3.0;
         }
         PCFreq = ((double)li.QuadPart) / 1000.0;
    }
    // printf("%s() PCFreq %9.6f MHz %9.6f\n",__FUNCTION__,
    //              PCFreq, DT_Mdep_GetCpuMhz());
    return PCFreq;
}
#endif
