...
dapl_os_atomic_assign (
    ...
    IN	DAT_COUNT new_value )
{
	return InterlockedCompareExchange((LPLONG)v, new_value, match_value);
}
    ...
    IN	DAPL_OS_WAIT_OBJECT *wait_obj)
{
    ...
    *wait_obj = (DAPL_OS_WAIT_OBJECT*)0;
...
{
    return (DAPL_OS_TIMEVAL) GetTickCount64(); // # of milliseconds since boot
}

STATIC __inline int dapl_os_ticks_to_seconds (DAPL_OS_TICKS ticks)
{
    return (ticks > 0 ? ticks / 1000 : 0);
}

STATIC __inline DAPL_OS_TIMEVAL dapl_os_time_stamp (void)
{
	LARGE_INTEGER	val;
...
#define dapl_os_assert(expression)  if (!(expression) ) DebugBreak()

#else

#define dapl_os_assert(exp)	\
  if (!(exp) ) {		\
	char msg[128];		\
	char fname[20];		\
	char ext[5];		\
	_splitpath(__FILE__,NULL,NULL,fname,ext);	\
        sprintf(msg,"DAPL Assertion Failed:" #exp " %s%s line#%d\n",fname,ext,__LINE__);\
