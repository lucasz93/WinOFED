    ...
    for (i = 0; i < p_ca_attr->num_ports; i++)
    {
...

        {
            ...
            if ( p_port )
            {
                ...
                /*
                 *  Remember the parent CA keeping the reference we took above
                 */
                p_port->ca = ca;
	...
	OUT DAT_SOCK_ADDR6                 *ip_addr)
{
    ...
    if (ip_addr != NULL)
    {
    	...
    	gid.gid_prefix =
            p_hca_attr->p_port_attr[port_num-1].p_gid_table->unicast.prefix;

    	gid.guid =
            p_hca_attr->p_port_attr[port_num-1].p_gid_table->unicast.interface_id;
    	...
    	if ( dat_status != DAT_SUCCESS )
    	{
            dapl_dbg_log (DAPL_DBG_TYPE_ERR, "%s() IBAT Query GID {%s} failed %#x\n",
                __FUNCTION__,
		dapli_gid_str((void*)&gid,NULL),
                dat_status);
    	...
    	else
    	{
    	    dapl_dbg_log (DAPL_DBG_TYPE_CM,
               "\n  %s()IBAT query GID {%s} for IP: %0d:%d:%d:%d\n", __FUNCTION__,
		dapli_gid_str((void*)&gid,NULL),
