...
dapli_ib_cm_event_str(ib_cm_events_t e)
{
    ...
    if (e > IB_CM_LOCAL_FAILURE || e < IB_CME_CONNECTED) {
	    _snprintf(msg,sizeof(msg),"Unknown EVENT %#x (%d)?",e);
        cp = msg;
    }
    else
        cp = event_str[e];
...
/*
 * CM reply callback - sent a REQ, now received a REP
 */
        ...
        IN    ib_cm_rej_rec_t          *p_cm_rej_rec )
{
    ...
    if ( p_cm_rej_rec->rej_status != IB_REJ_USER_DEFINED &&
    	p_cm_rej_rec->rej_status != IB_REJ_TIMEOUT )
    {
    	dapl_dbg_log (DAPL_DBG_TYPE_CM, 
                  "--> DiCRjcb: EP = %lx QP = %lx rej reason %#x(%d)\n", 
    ...
    switch (p_cm_rej_rec->rej_status)
    {
        ...
        case IB_REJ_TIMEOUT:
            ...
            dapl_dbg_log (DAPL_DBG_TYPE_CM, "--> DiCRjcb: REJ TIMEOUT\n");
            break;

        case IB_REJ_USER_DEFINED:
            cm_event = IB_CME_DESTINATION_REJECT;
            dapl_dbg_log (DAPL_DBG_TYPE_CM, 
                   "--> DiCRjcb: user defined rej reason '%s' (%d)\n",
                       p_cm_rej_rec->p_ari, CL_NTOH16(p_cm_rej_rec->rej_status));
            break;
    ...
    /* FIXME - Vu
     * We do not take care of the user defined rej reason with additional 
...
dapli_ib_cm_req_cb ( IN  ib_cm_req_rec_t  *p_cm_req_rec )
{
...
#if defined(DAPL_DBG)
    {
        ...
        dapl_dbg_log (DAPL_DBG_TYPE_CM|DAPL_DBG_TYPE_CALLBACK, 
            "%s() (CM %lx)->dst_ip_addr [%s]\n",
        	__FUNCTION__,cm_ptr,
		dapli_IPaddr_str((DAT_SOCK_ADDR*)&cm_ptr->dst_ip_addr, NULL));
    }
#endif
...
#if 0
	fprintf(stdout,"%s(%I64d) calling dapls_cr_callback()\n"
	    "  local [%s] sp_listening %s %s\n",
		__FUNCTION__,dapl_os_time_stamp(),
		dapli_IPaddr_str((DAT_SOCK_ADDR*)&cm_ptr->dst_ip_addr, NULL),
	    (sp_ptr->listening == DAT_FALSE ? "FALSE":"TRUE"),
 	((dapls_ib_get_dat_event(IB_CME_CONNECTION_REQUEST_PENDING,DAT_FALSE)
		== DAT_CONNECTION_REQUEST_EVENT) ? "DAT_CONN_REQ_EVT":"??"));
	fflush(stdout);
#endif
        ...
        IN        DAT_PVOID                    private_data )
{
    ...
    dapl_dbg_log (DAPL_DBG_TYPE_CM, "%s() IB connect to [%s]?\n",
		__FUNCTION__,dapli_IPaddr_str(remote_ia_address,NULL));
    /*
    ...
    dapl_os_memzero (&ep_ptr->remote_ia_address, sizeof (ep_ptr->remote_ia_address));

    /* dst larger than src, hence the above memzero. */
    dapl_os_memcpy (&ep_ptr->remote_ia_address, 
                    remote_ia_address, 
                    sizeof (*remote_ia_address));

    dat_status = dapli_IbatQueryPathRecordByIPaddr( remote_ia_address, &path_rec );
...
//#endif // NO_NAME_SERVICE
	...
	/* 
     * number of times local QP should retry after receiving RNR NACK before
     * reporting an error
     */
    cm_req.rnr_retry_cnt       = IB_RNR_RETRY_CNT;

    cm_req.remote_resp_timeout = ib_path_rec_pkt_life(path_rec);
    if ( cm_req.remote_resp_timeout < 18 )
        cm_req.remote_resp_timeout = 18;

    cm_req.local_resp_timeout = ib_path_rec_pkt_life(path_rec);
    if ( cm_req.local_resp_timeout < 18 )
    	cm_req.local_resp_timeout = 18;
        ...
        IN  DAPL_SP               *sp_ptr )
{
    ...
    dapl_dbg_log (DAPL_DBG_TYPE_CM, "%s: SP %lx port %d GID{%s} SID 0x%I64x\n", 
         __FUNCTION__,
         sp_ptr, p_active_port->p_attr->port_num,
         dapli_gid_str((void*)&p_active_port->p_attr->p_gid_table[0],NULL),
         cl_hton64 (ServiceID));
                             ...
                             IN  const DAT_PVOID     private_data)
{
    ...
    switch( reject_reason )
    {
      case DAT_CONNECTION_EVENT_UNREACHABLE: 
        strcpy(ari.data,"DAT_CONNECTION_EVENT_UNREACHABLE"); 
        break;
      case DAT_CONNECTION_EVENT_BROKEN: 
        strcpy(ari.data,"DAT_CONNECTION_EVENT_BROKEN"); 
        break;
      case IB_CM_REJ_REASON_CONSUMER_REJ: 
        strcpy(ari.data,"IB_CM_REJ_REASON_CONSUMER_REJ"); 
        break;
      default:
        _snprintf(ari.data,sizeof(ari.data),"Unknown reject reason %#x (%d)?",
            reject_reason,reject_reason);
        break;
    }

    cm_rej.rej_status   = IB_REJ_USER_DEFINED;
    cm_rej.p_ari        = (ib_ari_t*)ari.data;
    cm_rej.ari_length   = (uint8_t)strlen(ari.data);
    ...
    /* be tolerant of specific failures as the connection is alreay destroyed */
    if (ib_status == IB_INVALID_HANDLE || ib_status == IB_INVALID_STATE)
        ib_status = IB_SUCCESS;
    else if (ib_status != IB_SUCCESS)
    {
        dapl_dbg_log ( DAPL_DBG_TYPE_ERR,
        ...
        IN const  DAT_PVOID                private_data )
{
...
#if defined(DAPL_DBG)
    {
        dapl_dbg_log (DAPL_DBG_TYPE_CM|DAPL_DBG_TYPE_CALLBACK,
                  ...
                  __FUNCTION__, ep_ptr,
                  dapli_IPaddr_str((DAT_SOCK_ADDR*)&ep_ptr->remote_ia_address,NULL));
    }
#endif
    ...
    if (ib_status != IB_SUCCESS)
    {
    	...
    	if (ib_status == IB_INVALID_STATE)
            return DAT_INVALID_STATE_EP_DISCONNECTED;
    }
