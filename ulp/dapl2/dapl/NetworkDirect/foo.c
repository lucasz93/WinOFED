...
/* Typedefs map common DAPL provider types to NetworkDirect structs */
...
struct dapl_cm_id {
	...
	DAPL_ATOMIC			seen_disconn_callback;
	DAPL_ND_QP			*h_qp;
};
...
typedef struct _ib_hca_transport
{
	...
	/* device attributes */
	HANDLE			Event;		// general purpose event: set_notify.
...
DAPL_CM_ID *dapls_cm_id_create(struct dapl_hca *hca, HANDLE ovf);
